...
The separately archived internal-coordinate worm confirmation panel also evaluated D, yielding base-pair F1 of 0.794878 for seed 42 and 0.803820 for seed 17. This confirms the availability of internal CONF evidence, while the mixed absolute-threshold outcome remains consistent with unstable acceptance. Because worm participated in training, these results do not establish unseen-species transfer. [E05]
...
| **E05** 内部CONF | `docs/experiments/CROSS-SPECIES-L1-UPSTREAM-20260904-CONF.md`；同实验目录 `conf/seed{42,17}_D/conf_metrics.json` | 预先定义的worm内部坐标CONF；不属于未见物种测试 |
