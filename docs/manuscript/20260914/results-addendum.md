...
The initial synthetic fixed-commit Omni workflow validated integration and retained unsupported and blocked states. A subsequent real-region workflow used the same four fixed 1-MiB regions per species for native RepeatMasker, HiTE and RepeatModeler plus RepeatMasker, together with the three saved D predictions. A clean GitHub-pinned Omni replay completed four workflow jobs on a partial snapshot and retained all 12 expected cells: four had completed outputs, two were running and six had not yet produced artifacts. Unfinished metrics remained null. The replay measures concordance and coverage without independent truth; its collection time is separate from native caller runtime. The full native matrix is still in progress, so this is not a completed whole-genome accuracy or speed ranking. [Actual real-panel replay](../../experiments/TE-REAL-PANEL-BENCH-20260914-OMNI.md)
