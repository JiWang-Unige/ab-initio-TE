...
## 独立结构评价的执行定义（2026-09-25，目标预测前）

用已准备的长读长源exon坐标固定结构清单：至少一个对应源转录本的完整exon跨度必须位于一个连续允许区域，避免跨旧halo；按chrom/strand/完整intron链去重。主域与全常染色体域分别固定，不能把源文件全部48,086个结构直接当主域分母。报告全部观测结构、与完整RefSeq CDS内含子链相容子集，以及源class_code分层。u不等于coding，参考相容子集仍明确属于参考辅助。
