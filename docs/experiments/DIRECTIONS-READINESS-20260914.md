...
|---|---|---|
| Is the latest six-species D model evaluated on unseen species? | D has six-species DEV, a *C. elegans* SCREEN, and the already archived internal-coordinate *C. elegans* CONF result (F1 `0.794878`/`0.803820` for seeds 42/17). The `conf_evaluated=false` field belongs to DEV/SCREEN and does not negate the separate CONF artifacts. Subsequently, fixed four-region GPU inference completed for platypus, sea urchin and CB4 (`12696615`). Strict-label rescoring found platypus T1 positive recall `.959325`; sea urchin/CB4 have no known-TE positives in the frozen panel and are not evaluable. Horse/opossum/fly/cattle remain outside this execution. | A finite external diagnostic exists; no completed multi-species robustness or independent accuracy claim. See `DIRECTIONS-EXECUTION-20260914.md` and the D-RC0 actual report. |
