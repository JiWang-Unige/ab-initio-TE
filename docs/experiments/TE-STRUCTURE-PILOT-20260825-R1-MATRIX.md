...
2. P2-H is unblocked as **annotation-conditioned TE-aware continued pretraining**.
...
| P2-M | Explicit reference-run strata produce the intended continuous masking task | Span-mask mechanism only | Human train windows with explicit binary reference annotation | Split before mask construction/crop/augmentation | No family/copy field; homology leakage unknown and not claimable | Annotation-conditioned span-MLM builder/collator | disjoint eligible bases; actual selected bp by stratum; every boundary span crosses a known transition; zero N/pad/window-edge spans | Mechanism go only if masks satisfy the contract and actual masked bp is matched; otherwise P2-H is not submitted | CPU minutes | Audited Human H0 data | Correct implementation of annotation-conditioned masking | Biological TE boundary or copy identity |
| P2-H | Annotation-conditioned span continuation improves downstream reference-run structure | Masking distribution/objective geometry only; same Human windows, backbone, 8192 context, 800 steps and CE stage | Actual materialized train/val/test are chr1/chr11/chr17 prefixes; matched evaluation is first 1,200 chr17 windows; Mouse chr1 external; dm6 excluded | Coordinate split only; Mouse never tunes thresholds; family/homology leakage unknown | TE-aware span-MLM -> identical CE head | Human six gates; D1; Mouse transfer; FlyBase T1; stratum audit | Human must reach segment F1 >=0.400724, boundary F1@5 >=0.244286, short <=0.497931, fragments/truth <=1.180148, bp F1 >=0.929173 and missed <=0.087532. Human failure stops A | GPU <=8 h | P2-M | Increment from reference-annotation-conditioned continued pretraining on this exact Human screen | Biological copy-boundary learning, multi-chromosome generalization, unseen-family generalization or universal transfer |
