...
全60格12694349完成（9:49:37）；原评分12696406因把派生指标一并求和而失败，修复为仅汇总TP/FP/FN后，12710872评分及独立native CDS复算通过（10秒）。9项回归通过，20个preflight及60个原生输入观测齐全，配置与冻结版本相同，无参考core的FP仍保留。没有重跑模型或修改本协议。

726个参考locus上，U/P/R的F1为0.636307/0.699363/0.695431。P−U为+0.063056，固定配对区块bootstrap95%区间[0.036503,0.106832]，recall差+0.053719；新增55个正确locus，丢失16个。丢失比例16/510=3.1373%超过1%上限，因此前三门通过、最后一门不通过，联合决定为`P3_BASE_MASK_UTILITY_GATE_NOT_MET`。按本协议停止此固定问题；平均改善与局部损失必须同时报告，不改阈值补救。P−R区间跨零，不作优势或等效结论。

[完整结果、计数与修复证据](../../reports/P3-TIBERIUS-BASE-MASK-20260911-R1/full-r1-score-12710872/RESULTS.md)；[可复现论文图](../manuscript/20260914/figures/tiberius-base-mask-utility-caption.md)。
