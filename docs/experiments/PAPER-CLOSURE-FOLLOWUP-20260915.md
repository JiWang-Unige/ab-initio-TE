...
| 工作 | 作业/输出 | 后续验收 |
|---|---|---|
| 库控制 | 12732016、12732075；由库控制协议记录各 attempt 分工 | 同引擎/同 hg19 输入；区分全库与人 lineage 的内容混杂；no-reuse 匹配、未匹配分母、蛋白证据 |
| source-only 匹配/蛋白 | 12732331；具体重试以方向文档为准 | 新 no-reuse TN 必须进入蛋白对照；蛋白同源支持不等于生物真值 |
