#!/usr/bin/env python3
"""Train/evaluate a GENERanno superfamily token head from a binary checkpoint."""
...
def compute_metrics(eval_pred):
    ...
    for label, name in ID2LABEL.items():
        ...
        if label > 0:
            f_te.append(f)
    true_bin = (y_true != 0).astype(np.int8)
    pred_bin = (y_pred != 0).astype(np.int8)
    _, _, te_f1, _ = safe_prf(true_bin, pred_bin, 1)
    out["te_detect_f1"] = te_f1
    out["class_macro_f1"] = float(np.mean(f_te)) if f_te else 0.0
    out["macro_f1_all6"] = float(np.mean(f_all)) if f_all else 0.0
...
class WeightedTrainer(Trainer):
    def __init__(self, *args, class_weights=None, **kwargs):
        ...
        self.class_weights = torch.tensor(class_weights or [1.0, 3.0, 3.0, 3.0, 3.0, 2.0], dtype=torch.float32)
...
def train(args) -> None:
    ...
    targs = TrainingArguments(
        ...
        metric_for_best_model="class_macro_f1",
    ...
    if test_path.exists():
        ...
        result.update({"stage": "human_H0_superfamily", "window": args.window, "n_windows": len(test_ds)})
...
def evaluate(args) -> None:
    ...
    targs = TrainingArguments(output_dir=str(Path(args.out_json).parent / "_tmp_sf_eval"),
