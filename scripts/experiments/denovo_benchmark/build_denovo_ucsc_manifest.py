#!/usr/bin/env python3
"""Build B-animal eval-only de novo vs UCSC comparison manifest."""
...
def main() -> int:
    ...
    with ready_manifest.open() as handle, out.open("w", newline="") as out_handle:
        ...
        for row in reader:
            ...
            if not comparator or comparator == "NA":
                continue
                ...
                writer.writerow(
                    {
                        ...
                        "tool": tool,
                        "annotation_gff3": str(run_root / "raw_outputs" / species / tool / "annotation.gff3"),
                        "comparator_strict": comparator,
                    }
                )
    print(out)
    return 0
